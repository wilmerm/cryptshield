...
if [ $FILE_COUNT -eq 1 ]; then
    ...
    DIALOG_TEXT="Ingresa la clave para cifrar \"$(basename "$FIRST_FILE")\":"
else
    # Multiple files selected
    DIALOG_TEXT="Ingresa la clave para cifrar $FILE_COUNT archivos seleccionados:"
fi

# Solicita la clave de cifrado con zenity
KEY=$(zenity --entry --text="$DIALOG_TEXT" --title="Clave de cifrado")
...
while IFS= read -r file; do
    if [ -n "$file" ] && [ -e "$file" ]; then
        COMMANDS="${COMMANDS}echo 'Cifrando \"$file\"...';"
        COMMANDS="${COMMANDS}if command -v cryptshield >/dev/null 2>&1; then cryptshield encrypt \"$file\" \"$KEY\"; else python3 -m cryptshield encrypt \"$file\" \"$KEY\" 2>/dev/null || echo 'Error: cryptshield no está instalado correctamente'; fi;"
    elif [ -n "$file" ]; then
...
if [ -n "$COMMANDS" ]; then
    gnome-terminal -- bash -c "
        $COMMANDS
        echo 'Operación de cifrado completada.'

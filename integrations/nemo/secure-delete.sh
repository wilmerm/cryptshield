...
if [ $FILE_COUNT -eq 1 ]; then
    ...
    DIALOG_TEXT="¿Estás seguro de que deseas eliminar \"$(basename "$FIRST_FILE")\"?"
else
    # Multiple files selected
    DIALOG_TEXT="¿Estás seguro de que deseas eliminar $FILE_COUNT archivos seleccionados?"
fi

# Muestra una ventana de confirmación con zenity
zenity --question --text="$DIALOG_TEXT" --title="Confirmación de eliminación"
...
if [ $? -eq 0 ]; then
    ...
    while IFS= read -r file; do
        if [ -n "$file" ] && [ -e "$file" ]; then
            ...
            COMMANDS="${COMMANDS}if command -v cryptshield >/dev/null 2>&1; then cryptshield delete \"$file\"; else python3 -m cryptshield delete \"$file\" 2>/dev/null || echo 'Error: cryptshield no está instalado correctamente'; fi;"
        elif [ -n "$file" ]; then
    ...
    if [ -n "$COMMANDS" ]; then
        gnome-terminal -- bash -c "
            $COMMANDS
            echo 'Operación de eliminación completada.'
